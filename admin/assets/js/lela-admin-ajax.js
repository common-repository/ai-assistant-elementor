...
// ==================================================================
    ...
    var xhr,
        msg_strings = lela_admin_global_var.box_messages,
        msg_icons = lela_admin_global_var.box_icons;
...
// ===================================================================
    ...
    var infobox = '#lela-infobox',
        infobox_msg = '.lela-infobox-msg',
        infobox_loading = 'lela-infobox-loading';

    $(document).on('click', infobox, function (e) {
        if ($(infobox).is(':visible') && ($(e.target).is('.lela-info-inner') || $(e.target).is('.lela-close-infobox'))) {
    ...
    function show_message_error(ajax_data, response) {
        $(infobox_msg).html(lela_admin_global_var.box_icons.error + response.message);
        setTimeout(function () {
            $(infobox).removeClass(infobox_loading);
        }, 1500);
    }
...
// ==================================================================
    ...
    $(document).on('click', '#lela_settings_save, #lela_settings_reset', function () {

        var func = $(this).data('action'),
            reset = (func === 'lela_reset_settings') ? true : null,
            setting_data = get_settings_data(reset);

        Ajax_Helper({
            nonce: lela_admin_global_var.nonce,
            action: 'lela_admin_ajax',
            func: func,
            reset: reset,
    ...
    function show_save_settings_success(ajax_data, response, msg) {
        ...
        setTimeout(function () {
            $(infobox).removeClass(infobox_loading);
            if (response.content) {
                $('.lela-settings').html($(response.content).html());
                LELA_JS.init();
            }
        }, 800);
    }
